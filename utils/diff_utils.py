...
def ddim_sample_loop(model, w, timesteps, y_dim, ddim_alphas, ddim_alphas_prev, ddim_sigmas, stochastic=True, noisy_y=None):
    device = next(model.parameters()).device
    ...
    # else:
    #     y_t = stochastic * torch.randn_like(torch.zeros([batch_size, y_dim])).to(device) + noisy_y
    y_t = stochastic * torch.randn_like(torch.zeros([batch_size, y_dim])).to(device)
    ...
    for i, step in enumerate(time_range):
        ...
        y_t, pred_y0 = ddim_sample_step(model, y_t, w, t, index, ddim_alphas,
                                        ddim_alphas_prev, ddim_sigmas, noisy_y)
