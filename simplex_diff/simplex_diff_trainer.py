...
from simplex_utils import convert_to_simplex, scale, logits_projection
...
class Simplex_Trainer():
    def __init__(self, args, train_dataset, valid_dataloader, test_dataloader, w_dim, best_plm):
        ...
        self.optimizer = Adam(self.simplex_diffs.parameters(), lr=self.args.lr, \
        ...
        for epoch in range(self.args.diff_epochs):
            ...
            with tqdm(enumerate(train_loader), total=len(train_loader), desc=f'train diffusion epoch {epoch}', ncols=120) as pbar:
                ...
                for step, data_batch in pbar:
                    ...
                    for model_i in range(self.args.n_model):
                        ...
                        if uncertain_idx.numel() != 0 and (epoch > self.args.warmup_epochs):
                            ...
                            for sample_i in range(self.args.num_sample):
                                ...
                                with torch.cuda.amp.autocast():
                                    ...
                                    outputs = self.simplex_diffs[model_i].reverse_t(sample_y, \
                        ...
                        with torch.cuda.amp.autocast():
                            self.simplex_diffs[model_i].train()
                            ...
                            loss, logits = self.simplex_diffs[model_i].forward_t(current_y_model, w_model, x_embed)
            ...
            if (epoch % 1 == 0 and epoch >= self.args.warmup_epochs) or epoch == self.args.diff_epochs-1:
                ...
                if test_acc >= max_accuracy:
                    torch.save(self.simplex_diffs, f"best_multi_diffusion_{self.args.dataset}.pt")
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for test_batch_idx, data_batch in tqdm(enumerate(self.test_dataloader), total=len(self.test_dataloader), desc=f'Simplex Diffusion Sampling...', ncols=100):
                ...
                for model_i in range(self.args.n_model):
                    outpus = self.simplex_diffs[model_i].reverse_t(None, w[:,model_i,:], x_embed, generator=torch.Generator(device=self.args.device).manual_seed(self.args.seed))
                    pred_labels[model_i] = torch.softmax(outpus.simplex, dim=1)
