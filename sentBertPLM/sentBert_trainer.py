...
from sentBertPLM.sentBert import SentenceBertForClassification
...
class PLModel(nn.Module):
    ...
    def forward(self, sentences, labels=None):
        ...
        for i in range(num_models):
            output = self.models[i](sentences, labels)
...
class sentBert_Trainer(nn.Module):
    ...
    def train(self):
        ...
        for epoch in trange(self.args.epochs, desc="Epoch"): 
            ...
            for step, batch in enumerate(self.train_dataloader):
                ...
                loss_ce = self.model(sentences=b_sents, labels=b_labels)
                if torch.cuda.device_count() > 1:
            ...
            for batch in self.train_dataloader:
                ...
                with torch.no_grad():
                    outputs = self.model(b_sents)
                    for idx in range(len(outputs)):
                        logits = outputs[idx][0]
                        if train_embeds[idx] is None:
                            train_embeds[idx] = outputs[idx][-1].squeeze()
                            train_labels[idx] = b_labels
                            train_logits[idx] = F.softmax(outputs[idx][0], dim=-1)
                        else:
                            train_embeds[idx] = torch.cat((train_embeds[idx], outputs[idx][-1].squeeze()), 0)
            ...
            for idx in range(len(outputs)):
                if train_embeds_list[idx] is None:
            ...
            for batch in self.valid_dataloader:
                ...
                # Forward pass, calculate logit predictions
                    outputs = self.model(b_sents)
                    for idx in range(len(outputs)):
                        if eval_embeds[idx] is None:
                            eval_embeds[idx] = outputs[idx][-1].squeeze()
                            eval_labels[idx] = b_labels
                            eval_logits[idx] = F.softmax(outputs[idx][0], dim=-1)
                        else:
                            eval_embeds[idx] = torch.cat((eval_embeds[idx], outputs[idx][-1].squeeze()), 0)
            ...
            for idx in range(len(self.model.models)):
                if eval_embeds_list[idx] is None:
            ...
            for batch in self.test_dataloader:
                ...
                with torch.no_grad():
                    ...
                    for idx in range(len(outputs)):
                        if test_embeds[idx] is None:
                            test_embeds[idx] = outputs[idx][-1].squeeze()
                            test_labels[idx] = b_labels
                        else:
                            test_embeds[idx] = torch.cat((test_embeds[idx], outputs[idx][-1].squeeze()), 0)
            ...
            for idx in range(len(outputs)):
                if test_embeds_list[idx] is None:
